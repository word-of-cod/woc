...
def create_game(
    *,
    ...
    if event_date.date() < pool_entry.stage.start_date:
        raise ValueError(
            'Game date cannot be before the stage starts.'
        )

    if event_date.date() > pool_entry.stage.end_date:
        raise ValueError(
            'Game date cannot be after the stage ends.'
        )
